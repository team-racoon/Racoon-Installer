...
namespace sig {
    void installSigPatches () {
        bpcInitialize();
        try {
            std::string version = inst::util::readTextFromFile("sdmc:/nsh-version");
            if(version != ""){
                inst::ui::mainApp->CreateShowDialog("Version de NSH", "Version: " + version, {"common.ok"_lang}, true);
            }else{
                inst::ui::mainApp->CreateShowDialog("Version de NSH", "No se encontró ninguna version de NSH", {"common.ok"_lang}, true);
            }
